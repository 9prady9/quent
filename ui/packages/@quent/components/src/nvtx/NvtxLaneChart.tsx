    ...
    >
      ...
      {active.type === 'range' ? (
          ...
          <dd>
            {active.item.incomplete
              ...
              : active.item.observed_duration === null
                ? 'unavailable'
                : formatNvtxDurationNs(active.item.observed_duration)}
          </dd>
          <dt className="text-muted-foreground">start</dt>
          <dd className="font-mono whitespace-nowrap">
            {active.item.observed_start.toString()} ns
          </dd>
          <dt className="text-muted-foreground">end</dt>
          <dd className="font-mono whitespace-nowrap">
            {active.item.observed_end === null
              ? 'open at trace boundary'
              : `${active.item.observed_end.toString()} ns`}
          </dd>
        ...
        <dl className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1">
          ...
          <dd className="font-mono whitespace-nowrap">{active.item.timestamp.toString()} ns</dd>
          <dt className="text-muted-foreground">domain</dt>
          <dd className="break-words">{active.item.domain_name}</dd>
          <dt className="text-muted-foreground">category</dt>
          <dd className="break-words">{active.item.category_name ?? 'Uncategorized'}</dd>
    ...
    () =>
      lanes.map((lane, laneIndex) => ({
        ...
        ranges: lane.ranges.map(range => ({
          range,
          startMs: nvtxUnixNsToRelativeMs(range.display_start, referenceUnixNs),
          endMs: nvtxUnixNsToRelativeMs(range.display_end, referenceUnixNs),
        })),
        marks: lane.marks.map(mark => ({
          mark,
          timestampMs: nvtxUnixNsToRelativeMs(mark.timestamp, referenceUnixNs),
        })),
      })),
    [lanes, referenceUnixNs]
  );
