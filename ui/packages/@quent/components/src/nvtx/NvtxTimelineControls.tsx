...
import { formatNvtxDurationNs } from './NvtxLaneChart.utils';
...
export function NvtxTimelineControls({
  ...
  const toggleDomain = (entry: NvtxCatalogControlEntry, domainId: bigint) => {
    const current = selections[entry.contextId] ?? [];
    const existing = current.find(selection => selection.domain_id === domainId);
    if (existing) {
      onSelectionChange(
        entry.contextId,
        current.filter(selection => selection.domain_id !== domainId)
      );
      return;
    }
  ...
  const toggleCategory = (
    ...
    const withoutDomain = current.filter(selection => selection.domain_id !== domainId);
    onSelectionChange(
      entry.contextId,
                    ...
                    >
                      ...
                      <dl className="mt-1 grid grid-cols-2 gap-x-2 gap-y-1">
                        ...
                        <dd className="text-right font-mono">
                          {formatNvtxDurationNs(item.total_duration)}
                        </dd>
                        <dt>average</dt>
                        <dd className="text-right font-mono">
                          {item.observed_count === 0n
                            ? '—'
                            : formatNvtxDurationNs(item.avg_duration)}
                        </dd>
                        <dt>minimum</dt>
                        <dd className="text-right font-mono">
                          {item.observed_count === 0n
                            ? '—'
                            : formatNvtxDurationNs(item.min_duration)}
                        </dd>
                        <dt>maximum</dt>
                        <dd className="text-right font-mono">
                          {item.observed_count === 0n
                            ? '—'
                            : formatNvtxDurationNs(item.max_duration)}
                        </dd>
                      </dl>
