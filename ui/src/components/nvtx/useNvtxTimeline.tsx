...
export function useNvtxTimeline({
  ...
  const contextIds = useMemo(
    () =>
      (contextsQuery.data?.context_ids ?? []).filter(contextId => {
    ...
    [contextResources, contextsQuery.data?.context_ids, engineId, resourceIds]
  );
  const catalogQueries = useQueries({
    queries: contextIds.map(contextId => nvtxCatalogQueryOptions(contextId)),
  });
  ...
  const viewport = useMemo(
    () => nvtxViewportFromZoom(referenceUnixNs, zoomRange),
    [referenceUnixNs, zoomRange]
  );
  ...
  return useMemo(() => {
    ...
    contextIds.forEach((contextId, index) => {
      ...
      if (viewportQuery.data) {
        const domainItems = responseDomainItems(
          contextId,
          viewportQuery.data,
          referenceUnixNs,
          false
