...
enum CachedModel {
    Absent,
    Present(Box<NvtxModel>),
}

#[derive(Clone)]
struct NvtxModelCache {
    models: Cache<Uuid, Arc<CachedModel>>,
        ...
        self.models
            ...
            .or_try_insert_with(async move {
                tokio::task::spawn_blocking(move || match importer(context_id) {
                    Ok(Some(events)) => Ok(Arc::new(CachedModel::Present(Box::new(
                        NvtxModelBuilder::build(events),
                    )))),
...
async fn catalog(
    ...
    match state.cache.get(context_id).await?.as_ref() {
        ...
        CachedModel::Present(model) => Ok(Json(NvtxCatalog::from_model(model))),
    }
}
...
async fn viewport(
    ...
    match state.cache.get(context_id).await?.as_ref() {
        CachedModel::Absent => Err(NvtxServerError::NotFound),
        CachedModel::Present(model) => NvtxViewportResponse::from_model(model, request)
            .map(Json)
...
mod tests {
    ...
    #[tokio::test]
    async fn capture_model_catalog_and_viewport_are_cached_end_to_end() {
        ...
        let catalog: NvtxCatalog = serde_json::from_slice(&body).unwrap();
        assert_eq!(catalog.trace_start, 100);
        assert_eq!(catalog.trace_end, 200);
        ...
        let response = app
            .oneshot(
                Request::post(format!("/api/nvtx/contexts/{present}/viewport"))
        ...
        assert_eq!(viewport.statistics[0].total_duration, 100);
        assert_eq!(
    ...
    async fn absent_and_empty_streams_are_distinct() {
        ...
        let empty_response = app
            .clone()
            .oneshot(
                Request::get(format!("/api/nvtx/contexts/{empty}/catalog"))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        let absent_response = app
            .oneshot(
                Request::get(format!("/api/nvtx/contexts/{absent}/catalog"))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn malformed_paths_and_oversized_requests_are_rejected() {
        ...
        let invalid_uuid = app
            .clone()
            .oneshot(
                Request::get("/api/nvtx/contexts/not-a-uuid/catalog")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn importer_failures_are_redacted_and_isolated_by_context() {
        ...
        let failed = app
            .clone()
            .oneshot(
                Request::get(format!("/api/nvtx/contexts/{failing}/catalog"))
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
